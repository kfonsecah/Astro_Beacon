...
|----|-------|----------------------|------|
| V-01 | Permisos en app.json | `grep "cameraPermission" app.json` | commit |
| V-02 | Hook useImagePicker | `ls src/hooks/useImagePicker.ts` | commit |
| V-03 | Componente SpeciesDetail | `ls app/species/[id].tsx` | commit |
| V-04 | Componente IdentifyModal | `ls app/species/identify.tsx` | commit |
| V-05 | Ruta modal en _layout | `grep "presentation: 'modal'" app/_layout.tsx` | commit |

## 2. Integrated Test Suites (Nyquist Gaps)

| Req ID | Behavior | Test Command | Strategy |
|--------|----------|--------------|----------|
| AI-05 | Compresión de imagen | `npm test src/utils/image.test.ts` | Unit (Wave 0) |
